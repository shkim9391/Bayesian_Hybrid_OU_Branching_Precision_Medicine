...
def make_figure3(
    ...
    ax3.set_title(f"C. Trait vs clone count (Patient {patient_id})", fontweight="bold", fontsize=10)
...
# ---------------------------------------------------------------------
def main():
    # Load full longitudinal series table
    series_df = load_series(EXCEL_PATH, sheet_name=SERIES_SHEET)

    # Find all patients with at least 2 distinct x(t) timepoints
    eligible = (
        series_df.query("series == 'x'")
        .groupby("Patient_ID")["t"]
        .nunique()
    )
    eligible = eligible[eligible >= 2].index.tolist()

    print(f"[INFO] Generating Figure 4 for {len(eligible)} patients:")
    print("       ", eligible)
